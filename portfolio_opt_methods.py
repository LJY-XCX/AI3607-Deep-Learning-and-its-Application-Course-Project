...
class LSTMModel(torch.nn.Module):
    ...
    def get_opt_layer(self):
        ...
        rf = cp.Parameter(self.num_assets)
    ...
    def forward(self, seq, pred_len, mode='pred', rf=0.03, k=5,
        ...
        if mode == 'pred': # only predict the future price
            ...
            if self.opt_layer is None:
                self.opt_layer = self.get_opt_layer()

            weight, = self.opt_layer(
                probs,
                mu.unsqueeze(0).repeat((gumbel_sample_num, 1)),
                sqrt_cov.unsqueeze(0).repeat((gumbel_sample_num, 1, 1)),
                torch.full([self.num_assets], rf, dtype=torch.double, device=mu.device),
                ## solver arguments tuning
