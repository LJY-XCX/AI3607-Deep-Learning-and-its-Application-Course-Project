...
class LSTMModel(torch.nn.Module):
    ...
    def get_opt_layer(self, k):
        """
        Get a CVXPY differentiable optimization layer
        """
        varW = cp.Variable(k)
        paramTopK = cp.Parameter(k)
        paramMu = cp.Parameter(k)
        paramSqrtCov = cp.Parameter((k, k))
        rf = cp.Parameter()
    ...
    def forward(self, seq, pred_len, mode='pred', rf=0.03, k=10,
        ...
        if mode == 'pred': # only predict the future price
            return pred_seq
        elif mode == 'pto': # predict-then-optimize
            ...
            #print(f'Sharpe ratio on prediction: {compute_sharpe_ratio(mu, cov, weight, rf):.4f}')
            ...
            if weight is None: # solver error, return a trivial solution
                weight = torch.ones_like(mu) / mu.shape[0]
            return pred_seq, weight
        elif mode == 'jpo-old': # joint predict and optimize by Gumbel Sinkhorn TopK and optimization layer
            mu, cov = compute_measures(pred_seq)
            device = mu.device

            if k > 0:
                top_k_indices, probs_values, probs = gumbel_sinkhorn_topk(
                    top_k_select, k, max_iter=100, tau=.05,
                    sample_num=gumbel_sample_num, noise_fact=gumbel_noise_fact, return_value=True
                )
            else:
                probs = torch.ones_like(mu)

            #sqrt_cov = torch_sqrtm(cov)

            sqrt_cov = sqrt_newton_schulz_autograd(cov)

            sampled_mu = mu[top_k_indices[0]].to(device)

            sampled_sqrt_cov = torch.index_select(sqrt_cov, 0, top_k_indices[0]).to(device)            
            sampled_sqrt_cov = torch.index_select(sampled_sqrt_cov, 1, top_k_indices[0]).to(device)

            ## Concatenating 10 sampled values
            #sqrt_cov = sqrt_newton_schulz_autograd(cov)

            #sampled_mu = mu[top_k_indices[0]].to(device).reshape(1, k)

            #sampled_sqrt_cov = torch.index_select(sqrt_cov, 0, top_k_indices[0]).to(device)
            #sampled_sqrt_cov = torch.index_select(sampled_sqrt_cov, 1, top_k_indices[0]).to(device).reshape(1, k, k)

            #for i in range(1, gumbel_sample_num):

            #    mu_i = mu[top_k_indices[i]].to(device).reshape(1, k)
            #    sampled_mu = torch.cat([sampled_mu, mu_i], dim=0)

            #    sqrt_cov_i = torch.index_select(sqrt_cov, 0, top_k_indices[i]).to(device)
            #    sqrt_cov_i = torch.index_select(sqrt_cov_i, 1, top_k_indices[i]).to(device).reshape(1, k, k)
            #    sampled_sqrt_cov = torch.cat([sampled_sqrt_cov, sqrt_cov_i], dim=0)
            #    print(sampled_mu.shape, sampled_sqrt_cov.shape)


            assert not torch.isnan(sqrt_cov).any()

            if self.opt_layer is None:
                self.opt_layer = self.get_opt_layer(k)

            weight, = self.opt_layer(
                probs_values,
                sampled_mu.unsqueeze(0).repeat((gumbel_sample_num, 1)),
                sampled_sqrt_cov.unsqueeze(0).repeat((gumbel_sample_num, 1, 1)),
                torch.tensor(rf, dtype=torch.double, device=mu.device)
            )


            supp_weight = torch.zeros((gumbel_sample_num, self.num_assets)).squeeze(0).double().to(device)

            for sample_index in range(gumbel_sample_num):
                #print(top_k_indices[sample_index].shape, weight.shape)
                supp_weight[sample_index][top_k_indices[sample_index]] = weight[sample_index]

            supp_weight = torch.relu(supp_weight)
            supp_weight = supp_weight / supp_weight.sum(dim=-1, keepdim=True)

            if return_best_weight:
                sharpe = compute_sharpe_ratio(mu, cov, supp_weight, rf)
                supp_weight = supp_weight[torch.argmax(sharpe)]
                #weight = weight[torch.argmax(sharpe)]

            return pred_seq, supp_weight
            
        elif mode == 'jpo': # joint predict and optimize by Gumbel Sinkhorn TopK
            mu, cov = compute_measures(pred_seq)
            if k > 0:
